...
*Note:* without ```RL=false```, an additional RL phase would augment the supervised learning by treating reward as negative error. ```RL=false``` sets training to standard supervised-only classification to prevent this.
