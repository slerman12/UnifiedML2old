    ...
    with torch.no_grad():
        if critic.discrete:
            ...
            if actor.discrete and one_hot:
                ...
                next_actions = next_Pi.rsample(num_actions, batch_first=True)
                next_actions_log_probs = next_Pi.log_prob(next_actions.transpose(0, 1)).sum(-1).transpose(0, 1).flatten(1)
